"""
Cleaning the raw HDB resale data.

Input: data/raw/hdb_resale_raw.csv
Output: data/processed/clean_hdb.csv
"""
...
if __name__ == "__main__":
    ...
    # 4. Basic cleaning
    df.columns = df.columns.str.strip()          # trimming spaces
    ...
    # converting remaining_lease text to numeric years
    ...
    df = df[df["resale_price"] >= 20_000]        # dropping obvious typos
    ...
    # 7-a dropping original sparse text col
    df = df.drop(columns=["remaining_lease"])

    # 7-b snake casing everything
    df.columns = (
        df.columns
          .str.lower()
          .str.replace(r"[ /\-]", "_", regex=True)
          .str.replace(r"__+", "_", regex=True)   # collapsing doubles
    )

    # 7-c merging any duplicate cols created by renaming
